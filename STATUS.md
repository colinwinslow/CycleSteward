...
**Last updated:** 2026-06-16 (stale-meter guardrail implemented — packet 3/F5 done; STALE_METER fault on prolonged blind-while-charging; ADR-0013 accepted; 13 new tests; reviews OK)
**Phase:** Phase 2 - HA adapter
**Next bounded packet:** Real-HA smoke test (queued packet 4 below). Run the integration in a dev HA instance (or at minimum hassfest + config-flow exercise); HA itself is the single biggest untested surface — all current evidence is against mocks. No spec yet.
**Current readiness:** READY-FOR-NEXT-PACKET (packet 3 complete, verified, reviewed)
