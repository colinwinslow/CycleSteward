...
**Last updated:** 2026-06-09 (ADR-0012 accepted: all four algorithm questions resolved — duration estimation, probe cadence/fallback, margin policy, dynamic start-time)
**Phase:** Phase 1 - pure core (transitioning to HA adapter)
**Next bounded packet:** HA adapter slice — scaffold `custom_components/cyclesteward/` skeleton, wire entities/services from ADR-0011 surface, begin with `charge_mode` select + `session_state` sensor + core `SessionController` integration.
