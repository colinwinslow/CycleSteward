...
**Last updated:** 2026-06-09 (ADR-0012 drafted: probe transparency decided; algorithm questions captured)
**Phase:** Phase 1 - pure core (transitioning to HA adapter)
**Next bounded packet:** Resolve ADR-0012 open questions A–D (duration estimation, probe cadence/fallback, margin policy, dynamic start-time + WAITING_FOR_SCHEDULE); then HA adapter slice can begin.
...
- (g) ADR-0012 (draft): Resolve open questions A–D — duration estimation from profile, probe cadence/fallback, margin policy, dynamic start-time + WAITING_FOR_SCHEDULE interaction. Probe transparency requirement already decided. Blocks HA adapter slice.
