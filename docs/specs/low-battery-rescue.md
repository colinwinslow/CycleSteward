...
true empty, but it should outrank just-in-time waiting. The integration needs a
way to detect and add a small bounded charge without leaving the charger on
indefinitely.

## Behavior contract

The rescue controller:
...
1. Add probe policy model.
2. Add classifier using learned low-start band.
3. Add bounded rescue Wh/runtime.
4. Add relay-cycle guardrail.

## Proof requirements

1. Unit tests for low, normal, no-bike, and stale-meter probe outcomes.
